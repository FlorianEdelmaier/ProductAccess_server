...
exports.add = (name, description, link, linkUserManual, linkSupportSite, linkMarketingDoc, status, type) => {
    return db.run(`INSERT INTO Product (name, description, link, linkUserManual, linkSupportSite, linkMarketingDoc, status, type) VALUES ('${name}', '${description}', '${link}', '${linkUserManual}', '${linkSupportSite}', '${linkMarketingDoc}', '${status}', '${type}')`);
}

exports.update = (id, name, description, link, linkUserManual, linkSupportSite, linkMarketingDoc, status, type) => {
    return db.run(`UPDATE Product SET name = '${name}', description = '${description}', link = '${link}', linkUserManual = '${linkUserManual}', linkSupportSite = '${linkSupportSite}', linkMarketingDoc = '${linkMarketingDoc}', status = '${status}', type = '${type}' WHERE id = ${id}`);
}
